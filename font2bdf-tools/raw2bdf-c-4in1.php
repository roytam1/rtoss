...
$swidth = $w*90;
$bwidth=ceil($w/16);
for ($char=0; $char<$chrs*4; $char+=4)
{
	fwrite($fp,"STARTCHAR ".($char/4)."\n");
	fwrite($fp,"ENCODING ".($char/4)."\n");
	...
	for ($y=0; $y<$h*$bwidth/2; $y++)
	{
		...
		$v.=sprintf("%02X",$byteArray[$offset+$char*$skip+$char*($bwidth*$h/2)+$y]);
		$v.=sprintf("%02X\n",$byteArray[$offset+($char+1)*$skip+($char+1)*($bwidth*$h/2)+$y]);
		fwrite($fp, $v);
	}
	for ($y=0; $y<$h*$bwidth/2; $y++)
	{
		$v='';
		//printf("offset=%d, chr=%d\n",$char*($bwidth*$h)+$y,$byteArray[$char*($bwidth*$h)+$y]);
		$v.=sprintf("%02X",$byteArray[$offset+($char+2)*$skip+($char+2)*($bwidth*$h/2)+$y]);
		$v.=sprintf("%02X\n",$byteArray[$offset+($char+3)*$skip+($char+3)*($bwidth*$h/2)+$y]);
		fwrite($fp, $v);
	}
