	...
	for(i = 0; i < numcol; i++)
	{
		...
		if (ex_flg)
		{
			if (pCombocolor[i].defcol == 0xFFFFFFFF)
				col = GetMyRegLongEx("", pCombocolor[i].colname, col,confNo);
			else
				col = GetMyRegLongEx("", pCombocolor[i].colname, pCombocolor[i].defcol, confNo);
		}
		else
		{
			if (pCombocolor[i].defcol == 0xFFFFFFFF)
				col = GetMyRegLong("", pCombocolor[i].colname, col);
			else
				col = GetMyRegLong("", pCombocolor[i].colname, pCombocolor[i].defcol);
		}
