...
void ExecuteMouseFunction(HWND hwnd, LONG fnc, int btn, int clk)
{
	...
	switch (fnc)
	{
		...
		{
			KEYEVENT	key[]={
					{VK_LWIN, 0},					//Windowsキーを押す
					{VK_LWIN, KEYEVENTF_KEYUP},		//Windowsキーを離す
					{0xff,0},
			};
			PushKeybd(key);
			break;
		...
		case MOUSEFUNC_TASKSW:
		{
			KEYEVENT	key[]={
					{VK_MENU, 0},					//Altキーを押す
					{VK_SHIFT, 0},					//SHIFTキーを押す
					{VK_TAB, 0},					//TABキーを押す
					{VK_TAB, KEYEVENTF_KEYUP},		//TABキーを離す
					{VK_SHIFT, KEYEVENTF_KEYUP},	//SHIFTキーを離す
					{VK_MENU, KEYEVENTF_KEYUP},		//Altキーを離す
					{0xff,0},
			};
			PushKeybd(key);
			break;
