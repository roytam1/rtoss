	...
	switch(message)
	{
		...
		case WM_NOTIFY:
			switch(((NMHDR *)lParam)->code)
			{
				...
				case PSN_HELP: My2chHelp(GetParent(hDlg), 2); break;
			}
			return TRUE;
		case WM_DESTROY:
...
void OnInit(HWND hDlg)
{
	...
	nKaigyo = GetMyRegLong("", ENTRY(IDC_KAIGYO), -1);
	if(nKaigyo < 0)
	{
		RECT rc;
		HWND hwnd;
	...
	GetMyRegStrEx("", "Format", s, 1024, "", confNo);
	SetDlgItemText(hDlg, IDC_FORMAT, s);

	pCustomFormat = malloc(1024);
	if(pCustomFormat)
		GetMyRegStrEx("", "CustomFormat", pCustomFormat, 1024, "", confNo);

	On12Hour(hDlg);
	OnCustom(hDlg, FALSE);

	dw = GetMyRegLong(NULL, "IntervalSysInfo", 1);
	if(dw > 60) dw = 60;
	if(dw < 1 ) dw = 1;
	SendDlgItemMessage(hDlg,IDC_SYSIISPIN,UDM_SETRANGE,0,
		(LPARAM) MAKELONG((short)60, (short)1));
	SendDlgItemMessage(hDlg, IDC_SYSIISPIN, UDM_SETPOS, 0,
		(int)(short)dw);
}
...
void OnApply(HWND hDlg)
{
	...
	if(pCustomFormat)
	{
		if(IsDlgButtonChecked(hDlg, IDC_CUSTOM))
		{
			...
			SetMyRegStrEx("", "CustomFormat", pCustomFormat, confNo);
		}
	}

	dw = GetDlgItemInt(hDlg, IDC_SYSII, NULL, FALSE);
	if(dw > 60) dw = 60;
	if(dw < 0  ) dw = 0;
	SetDlgItemInt(hDlg, IDC_SYSII, dw, FALSE);
	SetMyRegLong("", "IntervalSysInfo", dw);
}
...
void InitFormat(void)
{
	...
	if(GetMyRegLong("", ENTRY(IDC_CUSTOM), FALSE))
		return;
	...
	for(i = IDC_YEAR4; i <= IDC_SECOND; i++)
	{
		CHECKS(i) = GetMyRegLong("", ENTRY(i), TRUE);
	}
	...
	CHECKS(IDC_KAIGYO) =
		GetMyRegLong("", ENTRY(IDC_KAIGYO), b);
	CHECKS(IDC_AMPM) = GetMyRegLong("", ENTRY(IDC_AMPM), FALSE);
