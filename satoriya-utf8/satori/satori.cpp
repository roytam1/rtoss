...
// エスケープ文字列
const char escaper::sm_escape_utf8_code[4]={(char)0xed,(char)0xa0,(char)0x80,0x00};
...
string escaper::insert(const string& i_str)
{
	...
	if ( it != m_id2str.end() ) {
		return string() + sm_escape_utf8_code + itos(distance(m_id2str.begin(),it)) + " ";
	}
	else {
		m_id2str.push_back(i_str);
		return string() + sm_escape_utf8_code + itos(m_id2str.size()-1) + " ";
	}
}
...
void escaper::unescape(string& io_str)
{
	...
	for (int i=0 ; i<max ; ++i)
		replace(io_str, string(sm_escape_utf8_code)+itos(i)+" ", m_id2str[i]);
}
...
void escaper::unescape_for_dic(string& io_str)
{
	const int	max = m_id2str.size();
	for (int i=0 ; i<max ; ++i)
		replace(io_str, string(sm_escape_utf8_code)+itos(i)+" ", "φ"+m_id2str[i]);
}
