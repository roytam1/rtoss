...
public class JOrbisPlayer extends JApplet implements ActionListener, Runnable{
  ...
  private JSObject myJS = null; //stores the window
  private JSObject theDocument = null; //stores the document
  ...
  public void init(){
    ...
    /* Get the JS window */
    myJS = JSObject.getWindow( this );
    /* Get the JS document */
    theDocument = (JSObject) myJS.getMember( "document" );
    ...
    s=getParameter("jorbis.player.callback");
    callback=s;
    ...
    loop: while(true){
...

      {
        ...
        for(int j=0; j<ptr.length; j++){
          ...
          String vcs;
          try {
          	vcs=new String(ptr[j], 0, ptr[j].length-1, "UTF-8");
          } catch(Exception e) {
          	String[] vcss = {"ERROR","Cannot recode to UTF-8."};
          	myJS.call( callback, vcss );
          	vcs=new String(ptr[j], 0, ptr[j].length-1);
          }
          System.err
              .println("Comment: "+vcs);
          if(sb!=null)
        ...
        System.err.println("Bitstream is "+vi.channels+" channel, "+vi.rate
            +"Hz");
        String vcs=new String(vc.vendor, 0, vc.vendor.length-1);
        System.err.println("Encoded by: "
            +vcs+"\n");
        if(sb!=null)
          acontext.showStatus(sb.toString());
        if(callback!=null) {
          String[] vcss = {"ENCODER",vcs};
          myJS.call( callback, vcss );
          vcss[0] = "TAG";vcss[1] = "End of TAG.";
          myJS.call( callback, vcss );
        }
      }
